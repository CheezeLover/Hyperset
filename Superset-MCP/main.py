...
@mcp.tool()
@handle_api_errors
async def superset_data_catalog_analyze(ctx: Context, question: str = None) -> Dict[str, Any]:
    """
    Analyze available data to answer questions intelligently.
    
    This tool provides a comprehensive catalog of all databases, datasets, and columns
    that the current user can access. The information is structured to help the LLM
    generate accurate SQL queries and create appropriate visualizations.
    
    Args:
        question: Optional - the specific question being answered (for context)
        
    Returns:
        Structured data catalog with databases, datasets, and detailed column information
        organized by type for optimal LLM utilization.
    """
    try:
        # Get user identity for context
        identity = extract_identity(ctx.request_context.request)
        
        # Step 1: Get all databases the user can access
        databases = await superset_request(ctx, "get", "/api/v1/database/")
        
        if "error" in databases:
            return {"error": f"Failed to load databases: {databases['error']}"}
        
        # Step 2: Get all datasets for each database
        catalog = {
            "user": identity.username,
            "email": identity.email,
            "roles": identity.roles,
            "question": question or "General data exploration",
            "databases": [],
            "total_datasets": 0,
            "total_tables": 0
        }
        ...
        for db in databases.get("result", []):
            ...
            datasets_resp = await superset_request(
                ctx, "get", f"/api/v1/database/{db_id}/tables/"
            )
            
            if "error" in datasets_resp:
                continue
            ...
            db_info = {
                "id": db_id,
                "name": db_name,
                "backend": db.get("sqlalchemy_uri", "").split(":")[0] if db.get("sqlalchemy_uri") else "unknown",
                "datasets": []
            }
            
            # Step 3: Get detailed column info for each dataset
            for dataset in datasets:
                ...
                # Get full dataset info including columns
                dataset_details = await superset_request(
                    ctx, "get", f"/api/v1/dataset/{dataset_id}/"
                )
                
                if "error" in dataset_details:
                    continue
                
                dataset_data = dataset_details.get("result", [])
                columns = dataset_data.get("columns", [])
                
                # Organize columns by type for LLM optimization
                datetime_cols = []
                numeric_cols = []
                text_cols = []
                other_cols = []
                ...
                for col in columns:
                    ...
                    # Categorize columns
                    if "time" in col_type or "date" in col_type or col.get("is_dttm"):
                        datetime_cols.append({
                            "name": col_name,
                            "type": col_type,
                            "is_dttm": True
                        })
                    elif any(t in col_type for t in ["int", "float", "double", "decimal", "number"]):
                        numeric_cols.append({
                            "name": col_name,
                            "type": col_type,
                            "is_metric": True
                        })
                    elif any(t in col_type for t in ["char", "text", "string", "varchar"]):
                        text_cols.append({
                            "name": col_name,
                            "type": col_type,
                            "is_dimension": True
        ...
        return {
            "success": True,
            "catalog": catalog,
            "message": f"Data catalog loaded: {catalog['total_datasets']} datasets across {len(catalog['databases'])} databases"
        }
        
    except Exception as e:
        return {
            "error": f"Failed to load data catalog: {str(e)}",
            "success": False
        }
